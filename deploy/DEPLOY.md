...
**Agar kod GitHub/GitLab'da bo'lsa** (tavsiya etiladi — keyingi yangilanishlar oson bo'ladi):

```bash
mkdir -p /var/www
cd /var/www
git clone <repo-url> takeavia_project
```

**Agar hali git repo yo'q bo'lsa** — kompyuteringizdan to'g'ridan-to'g'ri serverga
yuklashingiz mumkin: Windows'da **WinSCP** yoki **FileZilla** dasturi orqali
(SFTP protokoli, host: `95.217.128.222`, user: `root`) butun loyiha papkasini
`/var/www/takeavia_project` ichiga nusxalang. `db.sqlite3`, `venv/`, `__pycache__/`,
`staticfiles/`, `media/` papka/fayllarini yuklash shart emas — ular serverda
qaytadan yaratiladi.
